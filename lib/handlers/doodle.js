...
var doodle = module.exports = {
        ...
        return db.doodle.getBySlug(req.params.slug, function (err, doc) {
          ...
          if (doc) {
            if (data.checksum !== doc.checksum) {
              ...
              return doodle.fork(req.params.slug, data);
            }
  ...
  fork: function (slug, data, user) {
    if (typeof user === 'undefined') {
      console.log('Anonymous fork.');
    } else {
      console.log('Fork to a user (find the NEW user in the session)');
    }
  }
